...
bool winner(char role,int row ,int col){
	...
	int c=0;// check the hole colomn
	for(int i=0;i<N; i++){
		if(grid[i][col]==role) c++;
	}

	int dl=0;//check the hole left diagonal if the cell is located on it 
	if(row == col){
		for(int i=0;i<N;i++)
			if(grid[i][i]==role) dl++;
	}

	int dr=0;//check the hole rigtht diagonal if the cell is located on it 
